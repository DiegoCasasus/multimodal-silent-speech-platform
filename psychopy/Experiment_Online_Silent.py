...
expName = 'Experiment_Online_Silent'  # from the Builder filename that created this script
expVersion = ''
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    inst_silent_txt = visual.TextStim(win=win, name='inst_silent_txt',
        text='You will see one target word at the center of the screen.\n\nFirst it will appear in white.\nWhen it turns green, silently mouth that word.\n\nWait for feedback after each trial.\n\nPress SPACE to begin.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        ...
        if processing_stage == "failed": 
            ...
            if PREPARED_RUN is None: 
                ...
                cmd = [ 
                    ...
                    '--marker-stream-name', 'PsychoPy_Markers', 
                    '--mode', PREPARED_RUN.get('experiment_mode', 'online_silent'), 
                    '--device', 'cpu', 
                ] 
    ...
    # set up handler to look after randomisation of conditions etc
    trials_online_silent = data.TrialHandler2(
        name='trials_online_silent',
        nReps=45.0, 
        method='random', 
        extraInfo=expInfo, 
        originPath=-1, 
    ...
    thisExp.addLoop(trials_online_silent)  # add the loop to the experiment
    thisTrials_online_silent = trials_online_silent.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_online_silent.rgb)
    if thisTrials_online_silent != None:
        for paramName in thisTrials_online_silent:
            globals()[paramName] = thisTrials_online_silent[paramName]
    if thisSession is not None:
        # if running in a Session with a Liaison client, send data up to now
        thisSession.sendExperimentData()
    
    for thisTrials_online_silent in trials_online_silent:
        trials_online_silent.status = STARTED
        if hasattr(thisTrials_online_silent, 'status'):
            thisTrials_online_silent.status = STARTED
        currentLoop = trials_online_silent
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        if thisSession is not None:
            # if running in a Session with a Liaison client, send data up to now
            thisSession.sendExperimentData()
        ...
        while continueRoutine:
            # if trial has changed, end Routine now
            if hasattr(thisTrials_online_silent, 'status') and thisTrials_online_silent.status == STOPPING:
                continueRoutine = False
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        win.callOnFlip(outlet.push_sample, [f"SILENT_CUE:{target_word}"])
        ...
        win.callOnFlip(outlet.push_sample, [f"SILENT_GO:{target_word}"])
        ...
        outlet.push_sample([f"SILENT_END:{target_word}"])
        ...
        # Show a rest screen after every 50 completed online silent trials, except after the final trial.
        # This is safer for long validation runs: with 450 trials, this gives about 8 rests.
        break_every_n_trials = 50
        try:
            total_silent_trials = int(trials_online_silent.nTotal)
        except Exception:
            total_silent_trials = 0
        
        show_mini_break = (
            completed_online_trials > 0
            and completed_online_trials % break_every_n_trials == 0
            and (total_silent_trials <= 0 or completed_online_trials < total_silent_trials)
        )
        ...
        if not show_mini_break:
            ...
            force_window_focus(win)
            marker_txt = f"SILENT_REST_END:{completed_online_trials}"
            win.callOnFlip(outlet.push_sample, [f"SILENT_REST_START:{completed_online_trials}"])
            print(f"[REST] {completed_online_trials} silent trials completed. Press SPACEBAR to continue.", flush=True)
        ...
        if break_key.keys in ['', [], None]:  # No response was made
            break_key.keys = None
        trials_online_silent.addData('break_key.keys',break_key.keys)
        if break_key.keys != None:  # we had a response
            trials_online_silent.addData('break_key.rt', break_key.rt)
            trials_online_silent.addData('break_key.duration', break_key.duration)
        # the Routine "mini_break" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        # mark thisTrials_online_silent as finished
        if hasattr(thisTrials_online_silent, 'status'):
            thisTrials_online_silent.status = FINISHED
        # if awaiting a pause, pause now
        if trials_online_silent.status == PAUSED:
            thisExp.status = PAUSED
            pauseExperiment(
                thisExp=thisExp, 
                win=win, 
    ...
    # completed 45.0 repeats of 'trials_online_silent'
    trials_online_silent.status = FINISHED
