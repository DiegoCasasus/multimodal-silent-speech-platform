##python code\test_tools\fake_lsl_eeg_imu.py --eeg-name eeg_32_speech_eeg --eeg-type EEG --eeg-channels 32 --eeg-srate 256 --imu-name-prefix emg-imu_daux1 --imu-type IMU --imu-streams 4 --imu-channels-per-stream 9 --imu-srate 256
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--imu-srate", type=float, default=256.0)
...
def main() -> int:
    ...
    eeg_thread = PeriodicOutletThread(
        outlet=eeg_outlet,
        fs=args.eeg_srate,
        ...
        imu_threads.append(
            PeriodicOutletThread(
                ...
                make_sample_fn=lambda t, idx=i: make_imu_sample(
                    t, args.imu_channels_per-stream if False else args.imu_channels_per_stream, idx
                ),
    ...
    print(
        ...
        f"({args.eeg_channels} ch @ {args.eeg_srate} Hz)"
    )
    print(
        f"Started {args.imu_streams} fake IMU stream(s): "
        f"{args.imu_name_prefix}_1..{args.imu_streams} "
        f"({args.imu_channels_per_stream} ch each @ {args.imu_srate} Hz)"
    )
    print("Press Ctrl+C to stop.")

    eeg_thread.start()
    for th in imu_threads:
        th.start()
